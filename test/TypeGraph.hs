...
tests = do
  ...
  it "records a type synonym 1" $ do
     $([t|String|] >>= expandType >>= \string -> typeGraphInfo' [] [string] >>= lift . view synonyms) `shouldBe` (Map.fromList [(E (AppT ListT (ConT ''Char)), Set.singleton ''String)])

  it "records a type synonym 2" $ do
     $([t|String|] >>= expandType >>= \string -> withTypeGraphInfo' [] [string] (vertex Nothing string) >>= lift) `shouldBe` (TypeGraphVertex Nothing (singleton ''String) (E (AppT ListT (ConT ''Char))))

  it "can build the TypeInfoGraph for Type" $ do
    $(runQ [t|Type|] >>= expandType >>= \etyp -> typeGraphInfo' [] [etyp] >>= lift . pprint) `shouldBe` typeGraphInfoOfType
  ...
  it "can find the type synonyms in Dec (decTypeSynonyms)" $ do
     $(withLocalDeclarations [] $
       runQ [t|Dec|] >>= expandType >>= \typ -> withTypeGraphInfo [] [typ] (typeSynonymMapSimple >>= runQ . lift)) `shouldBe` decTypeSynonyms
#endif
